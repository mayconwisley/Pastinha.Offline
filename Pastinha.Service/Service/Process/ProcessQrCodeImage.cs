	...
	ICreateFolderLoose _createFolderLoose, IMoveFoderError _moveFoderError, IReturnFromTo _returnFromTo, CreateLog _createLog) : IProcessQrCodeImage
{
	...
	public async Task<DataQrCode?> ProcessSingleQrCodeMemory_1(Bitmap image, string pathError, List<(int dpi, int width, int height)> resizeAttempts)
	{
		try
		{
			_createLog.Log($"[INFO] Processando QRCode");
			ZXing.Result? result = null;
			var reader = new BarcodeReader
			{
				Options = new DecodingOptions
			...
			foreach (var (dpi, width, height) in resizeAttempts)
			{
				using var imageResize = _resizeImage.Image(image, dpi, width, height);
			...
			//Salvando imagem com erro na leitura do QRCode ou sem na pasta de ERRO.
			if (result is null)
			{
				var strName = $"{Guid.NewGuid():N}.png";
				var pathError_1 = Path.Combine(pathError, "Erro Leitura QRCode");

				if (!Directory.Exists(pathError_1))
				{
					Directory.CreateDirectory(pathError_1);
				}

				pathError_1 = Path.Combine(pathError_1, strName);

				_createLog.Log($"[AVISO] QRCode não encontrado, salvando imagem na pasta: {pathError}");
				image.Save(pathError_1);
