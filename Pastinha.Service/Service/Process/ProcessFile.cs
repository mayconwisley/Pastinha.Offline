	...
	IProcessFileExtension _processFileExtension, CreateLog _createLog) : IProcessFile
{
	...
	private async Task ProcessFileAsync(string file, string pathOutput, string pathError)
	{
		string _file = file;
		...
		if (extension == ".pdf")
		{
			await _processFilePdf.Process(_file, pathOutput, pathError);
		}
		else if (extension == ".png" || extension == ".jpg" || extension == ".jpeg" || extension == ".bmp")
		{
			await _processFileImage.Process(_file, pathOutput, pathError);
		}
		else
		{
	...
	public async Task Process(string path, string pathOutput, string pathError)
	{
		...
		try
		{
			...
			_createLog.Log($"[INFO] CPU: {maxParallelism}.");
			...
			{
				await ProcessFileAsync(file, pathOutput, pathError);
			});
